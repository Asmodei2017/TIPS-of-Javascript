...
- **Symbol** новый примитивный тип, предназначенный для уникальных идентификаторов. (ES6 читаем)[http://haa.su/EUG/]
...
- Если передать переменную , которая объявлена глобально в функцию как аргумент то функция не измени ее значение, а усли  в теле функции производить операции с переменной то ее значение ихмениться

```jsx
let a = 100;

function change(){
  a = 90; // изменяем значение глобальной переменной a
}
change();  // a=90; Функция изменила значение переменной

function changeWithArg(x){
  x=50;
}
changeWithArg(a); // вызвали функцию и передали аргумент глобальную переменную
//  a = 100 . Функция не повлияла на глобальную переменную
```
- Если вызвать метод удаления свойства из объекта delete даже если такого свойства нет , то он все равно вернет true, вернет false только в том случае , если не сможет удалить свойство которое есть 

```jsx
const a = { one : 22, two: 44};
delete a.one   // true
delete a.five  // true 
```
